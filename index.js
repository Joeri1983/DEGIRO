  ...
  if (req.method === 'GET') {
    ...
    https.get(azureStorageUrl, (response) => {
      ...
      response.on('end', () => {
        ...
        // Reverse the arrays to display values from newest to oldest
        const reversedDates = values.map((line) => line.date).reverse();
        const reversedValues = values.map((line) => line.value).reverse();
        ...
        res.write('var labels = ' + JSON.stringify(reversedDates) + ';');
        res.write('var data = ' + JSON.stringify(reversedValues) + ';');
